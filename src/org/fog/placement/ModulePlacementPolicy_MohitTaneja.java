...
public class ModulePlacementPolicy_MohitTaneja extends ModulePlacementPolicy {
	
	/**
	 * List of sensors considered for placement
	 */
	private List<Sensor> sensors;
	static int busylimit = 9;
	/**
	 * List of actuators considered for placement
	 */
	private List<Actuator> actuators;
	
	public ModulePlacementPolicy_MohitTaneja(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, Application application){
		super();
...
//-----FD SELECTION---------------------------------------------------------------------------------------		
		...
		FogDeviceCharacteristics selectedFogDevice = FindFog_Random(); //FindFog() //FindFog_BottomUpBase()
		
		while(true) {
			selectedFogDevice = FindFog();
			if(selectedFogDevice == null) {
				for (Integer fc : getFogDeviceCharacteristics().keySet()) {
					if (getFogDeviceCharacteristics().get(fc).isCloudDatacenter() == true) {
						selectedFogDevice = getFogDeviceCharacteristics().get(fc);
						System.out.println("SELECTED Cloud is " + selectedFogDevice.getResourceName());
						break;
					}
				}
			}else{
				System.out.println("SELECTED FOG is " + selectedFogDevice.getResourceName());
				break;
			}
			break;
		}
...
//-----FD SELECTION---------------------------------------------------------------------------------------		
		...
		for (int sensorId : getSensorCharacteristics().keySet()) {
			...
			for (AppModule module : getApplication().getModules()) {
				placement.addMapping(module.getName(), selectedFogDevice.getId());
			}
	...
	private FogDeviceCharacteristics FindFog() {
		FogDeviceCharacteristics selectedFogDevice = null;
		for (Integer fc : getFogDeviceCharacteristics().keySet()) {
			if (getFogDeviceCharacteristics().get(fc).isCloudDatacenter() == true) {
				
			}
			else if(getFogDeviceCharacteristics().get(fc).isBusy() < busylimit) {
				selectedFogDevice = getFogDeviceCharacteristics().get(fc);
				selectedFogDevice.setBusy();
				break;
			}	
		}
		return selectedFogDevice;
	}
	
	private FogDeviceCharacteristics FindFog_Random() {
		FogDeviceCharacteristics selectedFogDevice = null;
		for (Integer fc : getFogDeviceCharacteristics().keySet()) {
			if(getFogDeviceCharacteristics().get(fc).isBusy() < busylimit) {
				selectedFogDevice = getFogDeviceCharacteristics().get(fc);
				selectedFogDevice.setBusy();
				break;
			}
	...
	private FogDeviceCharacteristics FindFog_BottomUpBase() {
		FogDeviceCharacteristics selectedFogDevice = null;
		for (Integer fc : getFogDeviceCharacteristics().keySet()) {
			if (getFogDeviceCharacteristics().get(fc).isCloudDatacenter() == true) {
			}
			else if(getFogDeviceCharacteristics().get(fc).isBusy() < busylimit) {
				if(getFogDeviceCharacteristics().get(fc).getResourceName() == "FD20" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD21" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD22") {
					selectedFogDevice = getFogDeviceCharacteristics().get(fc);
					selectedFogDevice.setBusy();
					return selectedFogDevice;
				}	
		...
		for (Integer fc : getFogDeviceCharacteristics().keySet()) {
			if (getFogDeviceCharacteristics().get(fc).isCloudDatacenter() == true) {
				...
				if(getFogDeviceCharacteristics().get(fc).getResourceName() == "FD10" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD11" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD12" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD13" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD14" || getFogDeviceCharacteristics().get(fc).getResourceName() ==  "FD15") {
					selectedFogDevice = getFogDeviceCharacteristics().get(fc);
					selectedFogDevice.setBusy();
					return selectedFogDevice;
				}	
	...
	private FogDeviceCharacteristics FindFog_LocationBased() {
		FogDeviceCharacteristics selectedFogDevice = null;
		for (Integer fc : getFogDeviceCharacteristics().keySet()) {
			if (getFogDeviceCharacteristics().get(fc).isCloudDatacenter() == true) {
